...
### 2.3 테스트 3: 성능 개선 후 재테스트 (High Load)

**개선 사항 적용:**
1. HikariCP 풀 크기: 3 → 20
2. Redisson 커넥션 풀: 50 → 100
3. 대기열 추가 로직 Lua 스크립트 원자화
4. TokenExceptionHandler 추가 (중복 요청 시 HTTP 409 반환)
...
| 지표 | 개선 전 | 개선 후 | 변화 |
|------|---------|---------|------|
| 총 요청 수 | 807,052 | 689,030 | - |
| 평균 TPS | 4,479 | 4,500+ | 유지 |
| 평균 응답 시간 | 17ms | ~20ms | 유지 |
| 에러율 | **3.30%** | **0.01%** | **-99.7%** |

**분석:**
- Lua 스크립트로 Race Condition 해결
- HTTP 409 (Conflict)로 비즈니스 예외 분리 → 실제 시스템 에러만 에러율에 포함
- 에러율 3.3% → 0.01%로 대폭 개선 (100건/689,030건)
...
| 지표 | 목표 | 실측값 (개선 전) | 실측값 (개선 후) | 달성 여부 |
|------|------|-----------------|-----------------|----------|
| 응답 시간 P95 | < 500ms | ~50ms | ~50ms | **달성** |
| 응답 시간 P99 | < 1000ms | ~200ms | ~200ms | **달성** |
| 처리량 (TPS) | > 100 | 4,500+ | 4,500+ | **달성** |
| 에러율 (기본 부하) | < 0.1% | 0.00% | 0.00% | **달성** |
| 에러율 (고부하) | < 1% | 3.30% | **0.01%** | **달성** |
...
- 기본 부하에서 완벽한 안정성
- 평균 응답 시간 매우 우수
- **성능 개선 후 고부하에서도 안정적 (에러율 0.01%)**

**개선 완료:**
- ~~토큰 발급 API 고부하 취약~~ → Lua 스크립트 원자화로 해결
- ~~DB Connection Pool 작음~~ → HikariCP 20으로 증가 완료
- ~~Redisson 커넥션 풀 부족~~ → 100으로 증가 완료
...
초기 고부하 테스트에서 토큰 발급 API에 병목이 발생(에러율 3.3%)했으나, **성능 개선 후 에러율 0.01%로 대폭 개선**되었습니다.

### 적용된 개선 사항

| 항목 | 변경 전 | 변경 후 |
|------|---------|---------|
| HikariCP Pool Size | 3 | 20 |
| Redisson Connection Pool | 50 | 100 |
| 대기열 추가 로직 | check-then-add (Race Condition) | Lua 스크립트 (원자적) |
| 중복 요청 처리 | HTTP 500 | HTTP 409 (Conflict) |

**권장 동시 사용자 수: 730명 이상**에서도 안정적 운영 가능.
